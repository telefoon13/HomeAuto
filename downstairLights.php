<div class="row" id="R2">
    <div class="col-sm-2 col-6 align-self-center text-center" id="R2C1">
        <a href="index.php?page=switchlight&ip=192.168.178.150&port=1">
            <img src="img/downStairs/door.svg" alt="door" class="w-75">
            <h5>Inkom</h5>
        </a>
    </div>
    <div class="col-sm-2 col-6 align-self-center text-center" id="R2C2">
        <a href="index.php?page=switchlight&ip=192.168.178.150&port=2">
            <img src="img/downStairs/couch.svg" alt="couch" class="w-75">
            <h5>Zithoek</h5>
        </a>
    </div>
    <div class="col-sm-2 col-6 align-self-center text-center" id="R2C3">
        <a href="index.php?page=switchlight&ip=192.168.178.155&port=1">
            <img src="img/downStairs/bedside-table.svg" alt="bedside" class="w-75">
            <h5>Staande lamp</h5>
        </a>
    </div>
    <div class="col-sm-2 col-6 align-self-center text-center" id="R2C4">
        <a href="index.php?page=switchlight&ip=192.168.178.150&port=3">
            <img src="img/downStairs/abc-block.svg" alt="abc" class="w-75">
            <h5>Speelhoek</h5>
        </a>
    </div>
    <div class="col-sm-2 col-6 align-self-center text-center" id="R2C5">
        <a href="index.php?page=switchlight&ip=192.168.178.161&port=1">
            <img src="img/christmas-tree.svg" alt="christmas-tree" class="w-75">
            <h5>Kerstboom</h5>
        </a>
    </div>
    <div class="col-sm-2 col-6 align-self-center text-center" id="R2C6">
        <a href="index.php?page=switchlight&ip=192.168.178.150&port=4">
            <img src="img/downStairs/kitchen.svg" alt="kitchen" class="w-75">
            <h5>Keuken</h5>
        </a>
    </div>
</div>
<div class="row" id="R3">
    <div class="col-sm-2 col-6 align-self-center text-center" id="R3C1">
        <a href="index.php?page=switchlight&ip=192.168.178.151&port=1">
            <img src="img/downStairs/restaurant.svg" alt="restaurant" class="w-75">
            <h5>Eetkamer</h5>
        </a>
    </div>
    <div class="col-sm-2 col-6 align-self-center text-center" id="R3C2">
        <a href="index.php?page=switchlight&ip=192.168.178.151&port=2">
            <img src="img/downStairs/toilet-paper.svg" alt="toilet-paper" class="w-75">
            <h5>Toilet</h5>
        </a>
    </div>
    <div class="col-sm-2 col-6 align-self-center text-center" id="R3C3">
        <a href="index.php?page=switchlight&ip=192.168.178.151&port=3">
            <img src="img/downStairs/terrace.svg" alt="terrace" class="w-75">
            <h5>Terras</h5>
        </a>
    </div>
    <div class="col-sm-2 col-6 align-self-center text-center" id="R3C4">
        <a href="index.php?page=switchlight&ip=192.168.178.151&port=4">
            <img src="img/downStairs/shed.svg" alt="shed" class="w-75">
            <h5>Tuinhuis</h5>
        </a>
    </div>
    <div class="col-sm-2 col-6 align-self-center text-center" id="R3C5">
        <a href="index.php?page=hueSpots">
            <img src="img/downStairs/grass.svg" alt="grass" class="w-75">
            <h5>Tuinspots</h5>
        </a>
    </div>
    <div class="col-sm-2 col-6 align-self-center text-center" id="R3C6">
        &emsp;
    </div>
</div>